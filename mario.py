...
from tensorflow.keras import layers, models, losses, optimizers # lossesを追加
...
# --- 1. 脳みその設計図 (正規化とHuber Lossを追加) ---
def build_brain(action_size):
    inputs = layers.Input(shape=(84, 84, 4))
    x = layers.Rescaling(1./255)(inputs) # 🌟変更点1: ピクセル値を0〜1に正規化
...
class MarioAgent:
    def __init__(self, action_size):
        ...
        self.epsilon_decay = 0.99999975 # 🌟変更点4: 減衰をゆっくりに
        self.batch_size = 32

        self.burnin = 100000
        self.learn_every = 3
        self.sync_every = 10000 # 🌟追加: 何歩ごとにTarget Networkを更新するか
        self.curr_step = 0

    # 🌟追加: Target Networkを最新の脳みそと同期する関数
    ...
    def learn(self):
        ...
        # 🌟変更点5: 現在のQ値(targets)はbrainから、目標のQ値はtarget_brainから計算する
        targets = self.brain.predict(states, verbose=0)
        next_q_values = self.target_brain.predict(next_states, verbose=0) # ここが変わる！
...
# ==========================================
def main():
    print(">>> マリオの世界を構築中... (真・完成版)")
    env = gym_super_mario_bros.make('SuperMarioBros-1-1-v0')
    
    # 🌟 行動は「Bダッシュ」と「Bダッシュジャンプ」のみ！
    CUSTOM_MOVEMENT = [
        ['right', 'B'],      
        ['right', 'A', 'B']  
    ]
